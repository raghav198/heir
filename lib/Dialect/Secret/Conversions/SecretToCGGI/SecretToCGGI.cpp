...
class SecretGenericOpLUTConversion
  ...
  LogicalResult matchAndRewriteInner(
    ...
    // for (auto attr : truthOp->getAttrs()) llvm::dbgs() << attr.getName() << "
    // -> " << attr.getValue() << "\n";

    if (truthOp->hasAttr("coefficients") && truthOp->hasAttr("prepped_lut")) {
      auto newOp =
          rewriter.replaceOpWithNewOp<cggi::LutLinCombOp>(op, encodedInputs);
      ...
      rewriter.replaceOpWithNewOp<cggi::Lut2Op>(
          op, encodedInputs[0], encodedInputs[1], truthOp.getLookupTable());
      ...
      newOp->setAttr("coefficients",
                     rewriter.getDenseI32ArrayAttr(coefficients));
